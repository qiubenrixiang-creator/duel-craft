...
BGMは `public/bgm/` に置いた音声ファイルを再生している。
用意しているのは `bgm1.mp3` / `bgm2.mp3` / `bgm3.mp3` の3曲。

| 場面 | 鳴り方 |
| --- | --- |
| 対戦中 | 3曲をシャッフルし、1曲終わると次の曲へ進む |
| ホーム画面 | 3曲から1曲をランダムに選んでループ |
| 決着後 | 3曲から1曲をランダムに選んでループ |

同じ曲が2回続かないようになっている。
曲を変えたいときは同名のファイルを置き換えるだけでよい。
曲数を増やす場合は、ファイルを足して `src/ui/sound.ts` の `TRACKS` に名前を追加する。
場面ごとの音量は同じファイルの `MOOD_GAIN` で調整できる。

なお、スマートフォンでは「画面を一度タップするまで音を鳴らせない」という
ブラウザの制限があるため、最初のタップまで再生は保留される。
またiPhoneはマナーモード(消音スイッチ)が入っていると音が出ない。

---

## ライセンスと権利について

このプロジェクトは、既存のカードゲームの画像・イラスト・ロゴといった
著作物を一切含んでいません。背景はすべてCSSで描画したオリジナル、
効果音はWeb Audio APIによる自動生成です。
BGMは制作者が用意した音源を `public/bgm/` に配置している。
