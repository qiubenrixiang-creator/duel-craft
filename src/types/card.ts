...
/**
 * レアリティ。パックからの排出率と、カードの見た目(枠・演出)に影響する。
 * ゲームの強さには一切影響しない(案Aの方針)。
 */
export type Rarity = 'N' | 'R' | 'SR' | 'VR' | 'SEC';
...
export interface Card {
  ...
  /**
   * シークレット版のイラスト。
   * 設定すると、同じカードの別バージョンとして図鑑に並ぶ。
   * 画像を2枚持つことになるため、容量に注意する。
   */
  secretImage?: string;
  /** シークレット版のフレーバー(通常版と変えたい場合) */
  secretFlavor?: string;
}
